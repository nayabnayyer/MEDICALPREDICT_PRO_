...
if uploaded_file is not None:
    ...
    # Save original region names before encoding
    original_regions = data['region'].unique().tolist()
    ...
    region = st.selectbox("Region", original_regions)
    ...
    for reg in original_regions:
        col_name = f"region_{reg}"
        user_input[col_name] = [1 if region == reg else 0]
